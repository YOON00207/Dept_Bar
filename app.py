...
if not st.session_state.selected.empty:
    ...
    if st.toggle("데이터 출처 보기"):
        ...
        |------|------|----------------------|------|--------|
        | 신입생 충원율(%) | [대학알리미] 공시 데이터 다운로드 → 4-다. 신입생 충원 현황 학과별 다운로드 | 정원내 신입생 충원율(%) (D/B) × 100 | 좌측 열 그대로 반영 | B=모집인원_정원내, D=입학자_정원내 |
        | 신입생 경쟁률(%) | [대학알리미] 공시 데이터 다운로드 → 4-다. 신입생 충원 현황 학과별 다운로드 | 경쟁률(C/B) | 좌측 열 그대로 반영 |  |
        | 재학생 충원율(%) | [대학알리미] 공시 데이터 다운로드 → 4-라. 학생 충원 현황 학과별 다운로드 | 정원내 재학생 충원율(%){D/(A-B)}×100 | 대학알리미 기준 학과별 자료 X (계열별만 존재) | A=학생정원, B=모집정지인원, D=재학생 정원내 |
        | 중도탈락률(%) | [대학알리미] 공시 데이터 다운로드 → 4-사. 중도탈락학생현황 학과별 다운로드 | 중도탈락학생비율(%) (B/A)×100 | 좌측 열 그대로 반영 | A=재적학생, B=중도탈락학생 계 |
        | … | … | … | … | … |
