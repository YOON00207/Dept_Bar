...
if not st.session_state.selected.empty:
    ...
    if st.toggle("데이터 출처 보기"):
        ...
        |------|------|----------------------|------|--------|
        | 신입생 충원율(%) | 대학알리미 4-다 (신입생 충원 현황) | 정원내 신입생 충원율(%) (D/B) × 100 | 좌측 열 그대로 반영 | B=모집인원_정원내, D=입학자_정원내 |
        | 신입생 경쟁률(%) | 대학알리미 4-다 (신입생 충원 현황) | 경쟁률 (C/B) | 좌측 열 그대로 반영 |  |
        | 재학생 충원율(%) | 대학알리미 4-라 (학생 충원 현황) | 정원내 재학생 충원율(%){D/(A-B)}×100 | 계열별 자료 사용 | 2024년도 파일에는 2023하반기·2024상반기 / 2023년도 파일에는 2023상반기·2022하반기 존재하여<br>2023년 상·하반기 각각 추출해서 평균<br><br>A=학생정원, B=모집정지인원, D=재학생 정원내 |
        | 중도탈락률(%) | 대학알리미 4-사 (중도탈락학생 현황) | 중도탈락학생비율(%) (B/A)×100 | 좌측 열 그대로 반영 | A=재적학생, B=중도탈락학생 계 |
        | 캡스톤디자인 평균이수학생수(명) | 대학알리미 캡스톤디자인 파일 | 이수 학생수(명)_해당학과 | 학과(주간)별 1,2학기 평균 |  |
        | 졸업생 취업률(%) | 대학알리미 5-다 (졸업생 취업 현황) | 취업률(%) [B/{A-(C+D+E+F+G)}]×100 | 좌측 열 그대로 반영 | A=졸업자, B=취업자, C=진학자, D=입대자, E=취업불가능자, F=외국인유학생, G=제외인정자 |
        | 졸업생 진학률(%) | 대학알리미 5-다 (졸업생 취업 현황) | 졸업자(A), 진학자(C) | 좌측 열 그대로 반영 | 진학률 = 진학자 (남+여) / 졸업자(남+여) |
        | 교원 1인당 연구(논문) 실적(건) - 연구재단등재지 | 대학알리미 7-가 (연구 실적) | 전임교원 1인당 논문 실적_연구재단등재지(후보포함) | 좌측 열 그대로 반영 |  |
        | 교원 1인당 연구(논문) 실적(건) - SCI급/SCOPUS | 대학알리미 7-가 (연구 실적) | 전임교원 1인당 논문 실적_SCI급/SCOPUS학술지 | 좌측 열 그대로 반영 |  |
        | 교원 1인당 연구비 수혜 실적(천원) | 대학알리미 12-가 (연구비 수혜 실적) | 전임교원수, 연구비 계, 대응자금 | 좌측 열 그대로 반영 | (연구비 지원 계(교내+교외) + 대응자금) / 전임교원수 |
